# Description: This script processes JSON data from Azure Blob Storage and write the results to a CSV file.
import sys
import os
...
def main():
    ...
    # Define the path prefix for the subfolder structure in the Blob container.
    path_prefix = app_settings.blob_path_prefix
    print(f"Processing blobs in the container with path prefix: {path_prefix}")

    # Create manifest of input files from the blob names.
    ...
    # Prepare the CSV file for output with headers.
    with open(app_settings.output_csv_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for blob in list_blobs(blob_service_client, app_settings.blob_container_name, path_prefix):
            ...
            print(f"Read content for {blob_name}")
    
            if 'text_content' in content and content['text_content'].strip():
            # Check if key 'text_content' exists AND is not empty or whitespace-only.
            # 'strip()' removes leading/trailing whitespace, making this check fail for     empty or whitespace-only content.
                ...
                # clickable_path = app_settings.original_path + blob_name # TODO: Add clickable path to the output

                extracted_info = process_text(text_content, rulesets) 
                # Calls the process_text function from text_processing.py, that uses flashtext to extract information based on rulesets, and also uses nltk to tokenize the text into sentences.
                for match in extracted_info:

                    # Use list comprehension to compile the surrounding context for the current match in a list.
